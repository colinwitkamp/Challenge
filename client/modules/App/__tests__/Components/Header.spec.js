...
test('toggleAddOffer called properly', t => {
  const router = {
    isActive: sinon.stub().returns(true),
  };
  const toggleAddOffer = sinon.spy();
  const wrapper = shallow(
    <Header switchLanguage={() => {}} intl={intlProp} toggleAddOffer={toggleAddOffer} />,
    {
      context: {
        router,
        intl,
  ...
  t.truthy(toggleAddOffer.calledOnce);
});
